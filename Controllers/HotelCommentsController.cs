...
namespace ToursWebAPI.Controllers
{
    public class HotelCommentsController : ApiController
    {
        ...
        {
            return Ok(db.HotelComment.ToList().Where(p => p.HotelId == hotelId));
        }
        ...
        public IHttpActionResult PostHotelComment(HotelComment hotelComment)
        {
            ...
            if (string.IsNullOrWhiteSpace(hotelComment.Author))
                ModelState.AddModelError("Author", "Author is required string.");
